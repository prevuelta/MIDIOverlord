//
//  constants.h
//  MIDIOverlord
//
//  Created by Pablo Revuelta on 4/02/14.
//  Copyright (c) 2014 Midnight City. All rights reserved.
//

// Skins
#define MO_BLACK {34, 34, 34, 255}
#define MO_DARKEST_GREY {42, 42, 42, 255}

#define MO_DARK_BROWN {66, 49, 35, 255}
#define MO_BROWN {149, 149, 122, 255}
#define MO_BROWN_WHITE {244, 244, 233, 255}
#define MO_BLUE {0, 169, 232, 255}
#define MO_RED {237, 50, 0, 255}
//
//#define MO_YELLOW {231, 229, 29, 255}
//
//#define UI_COLOR_PROT_0 {200, 200, 200, 255}
//
#define UI_COLOR_PROT_1 {60, 60, 60, 255}
//#define UI_COLOR_PROT_2 {80, 80, 80, 255}
//#define UI_COLOR_PROT_3 {120, 120, 120, 150}



// Rack vars
#define MODULE_HEIGHT 42
#define BASE_UNIT 16
#define UNIT_WIDTH BASE_UNIT * 6
#define UNIT_HEIGHT BASE_UNIT * 4
#define SCROLLER_WIDTH 4
#define TITLE_BAR_HEIGHT BASE_UNIT * 4
#define TOOLBAR_HEIGHT BASE_UNIT * 2
#define WINDOW_HEIGHT 800